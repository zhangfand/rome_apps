...
the auth source (ROME_TOKEN or 24h session), and the handles you can publish
under.`;
...
function readJwtExpiryMs(token) {
    ...
    try {
        ...
        const json = JSON.parse(Buffer.from(padded + "=".repeat((4 - (padded.length % 4)) % 4), "base64").toString("utf8"));
...
export async function runWhoami(argv) {
    ...
    const body = await getJson(bearer.host, "/api/store/me", bearer);
