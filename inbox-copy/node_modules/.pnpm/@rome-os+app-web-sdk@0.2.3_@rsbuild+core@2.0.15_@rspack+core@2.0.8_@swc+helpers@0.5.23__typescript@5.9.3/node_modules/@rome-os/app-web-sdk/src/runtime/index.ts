...
export interface RomeAppGlobalParams {
  ...
  interaction?: boolean;
  /** Verified Rome Cloud visitor identity for public/restricted app surfaces. */
  viewer?: {
    accountId: string;
    email: string;
  };
...
export function getViewer(): RomeAppGlobalParams["viewer"] | null {
...
export function useViewer(): RomeAppGlobalParams["viewer"] | null {
...
export type NavigateRomePayload =
  | { readonly path: "chat"; readonly sessionId: string }
  /**
   * `draft` pre-fills the chat composer without sending — the guardian edits
   * and commits the message themselves (RFC 031 §4). `skill` pre-pins a
   ...
   * never parsed out of the text.
   */
  | { readonly path: "chat/new"; readonly draft?: string; readonly skill?: string }
 ...
 * drains in the background, RFC 013 §4.5), so this only costs the server's
...
export interface StartChatOptions {
  ...
  /**
   * Invoke a catalog skill on the first turn (RFC 031), with `message` as its
...
//
// An app can render its own interactive components *inline* inside the webchat
// transcript, instead of (or in addition to) a full-page surface. The app
// declares each component's id in app.yaml (`components: [...]`) so core can
// validate which component an action is allowed to render, and registers a
// renderer here at bundle-load time via `defineComponent`. When an action
// returns `pendingInteraction` with `render.kind: "inline"`, the host mounts
// this bundle in a chat-row shadow root and asks the generated entry's
// `mountComponent` to invoke the matching renderer.
//
// Unlike a `workspace` interaction surface (an iframe that talks back over
// postMessage), an inline component runs in the same page context as the host,
// so the host hands it a direct `AppComponentHost` bridge — no postMessage.
