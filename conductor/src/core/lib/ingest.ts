 ...
 *
 * A source — a poll, an order system, a mailbox, a person with curl —
 ...
 *    sees the same item twice, two adapters watching one system — all land on
 ...
 * the same batch is reported as a duplicate — a poll that lists the same item
...
export function planIngest(input: {
   ...
   * the task that holds them. The seam knows nothing about why they are claimed.
   */
