...
import { heartbeatState, type WorkerHeartbeat } from "../../lib/worker-health.js";
...
export class WorkerHealthRepository {
  ...
  claim(started: DispatchedFact, ownerId: string, leaseMs: number, now = new Date()): boolean {
    return this.db.transaction((tx) => {
      ...
      if (health.peek(started.payload.workerId) || heartbeatState(current, undefined, now, leaseMs).status === "expired") return false;
      tx.insert(this.tables.workerHealth).values({
        taskId: started.taskId, workerId: started.payload.workerId, ownerId,
        lastHeartbeatAt: now.getTime(), expiresAt: now.getTime() + leaseMs,
      }).run();
      return true;
    }, { behavior: "immediate" });
  }

  renew(taskId: string, workerId: string, ownerId: string, leaseMs: number, now = new Date()): boolean {
    return this.db.transaction((tx) => {
      ...
      return tx.update(this.tables.workerHealth)
        .set({ lastHeartbeatAt: now.getTime(), expiresAt: now.getTime() + leaseMs })
        .where(and(
   ...
   */
  expire(taskId: string, workerId: string, leaseMs: number, now = new Date()): Fact | undefined {
    return this.db.transaction((tx) => {
      const connection = tx as unknown as DrizzleDb;
      const ledger = new LedgerRepository(connection, this.tablePrefix);
      ...
      if (heartbeatState(started, heartbeat, now, leaseMs).status !== "expired") return undefined;
      const why = heartbeat
  ...
  status(leaseMs: number, now = new Date()) {
    ...
    return fold(now, ledger.all()).tasks.flatMap((task) => {
      ...
      return [{ taskId: task.id, workerId: started.payload.workerId,
        ...heartbeatState(started, this.peek(started.payload.workerId), now, leaseMs) }];
    });
  }
}
