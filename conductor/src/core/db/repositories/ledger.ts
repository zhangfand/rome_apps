...
export class LedgerRepository {
  ...
  appendWorkerOutcome(fact: NewFact & { payload: { workerId: string } }): Fact | undefined {
    ...
    return this.db.transaction((tx) => {
      ...
      // A closed task takes no more worker outcomes. A Reply after the
      // dispatch does not drop the outcome: the orchestrator sees both and
      // decides what the reply means for the result.
      if (history.some((f) => f.kind === "Completed" || f.kind === "Cancelled")) return undefined;
