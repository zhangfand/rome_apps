...
export class SettingsRepository {
  ...
  constructor(
    ...
    tablePrefix: string,
    private readonly parseConfig: ConfigParser,
  ) {
  ...
  get(): ConductorConfig | undefined {
    ...
    const parsed = this.parseConfig(JSON.parse(row.value));
...
export function createSettingsRepository(ctx: AppDbContext, parseConfig: ConfigParser): SettingsRepository {
  return new SettingsRepository(ctx.connection, ctx.tablePrefix, parseConfig);
}
