...
export function createAction(config: ActionConfig, deps: AppActionRuntimeDeps, composition: CoreComposition): Action {
  ...
  return {
    ...
    async execute(args): Promise<ActionResult> {
      ...
      let restarted = false;
      try {
        const run = await summonWithFallback({ appContext, ledger, dispatched, source, composition });
...
async function summonWithFallback(input: {
  ...
  try {
    ...
    // never a quiet fall back to the shared environment.
    if (!workspace) throw new Error("Worker has no workspace recorded; refusing to launch");
    await input.composition.providerFor(workspaceKind(workspace, input.composition.defaultWorkspaceKind)).validate(workspace);
  } catch (err) {
