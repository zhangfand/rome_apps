import type { AdapterPollContext, AdapterPollResult, SourceAdapter } from "../../lib/adapters.js";
import type { ConductorConfig } from "../../lib/config.js";
import type { LedgerSnapshot } from "../../lib/fold.js";
import type { IngestRequest } from "../../lib/ingest.js";
...
export const githubAdapter: SourceAdapter = {
  ...
  enabled(config: ConductorConfig): boolean {
    return Object.values(config.projects).some((project) => Boolean(project.repo));
  },
...
async function pollIntake(ctx: AdapterPollContext): Promise<IngestRequest[]> {
  ...
  for (const { repo, labels } of polls) {
    for (let page = 1; page <= MAX_INTAKE_PAGES; page += 1) {
      const rows = await get(ctx, intakeApiPath(repo, labels.join(","), page), { repo, page });
  ...
  return intakeRequests({ issues, label: ctx.config.intakeLabel, routes });
}
...
async function pollIssueCloses(ctx: AdapterPollContext): Promise<IngestRequest[]> {
  ...
  for (const ref of wanted.values()) {
    const issue = (await get(ctx, issueApiPath(ref), { issue: ref.url })) as Record<string, unknown> | undefined;
  ...
  for (const watch of watches) {
    ...
    for (const ref of watch.refs) {
      ...
      if (!seen) {
        ...
        const pull = (await get(ctx, paths.pull, { pull: ref.url })) as Record<string, unknown> | undefined;
        if (!pull) continue;
        seen = {
        ...
        const reviews = await get(ctx, paths.reviews, { pull: ref.url });
        ...
        const rcs = await get(ctx, paths.reviewComments, { pull: ref.url });
        ...
        const ics = await get(ctx, paths.comments, { pull: ref.url });
        if (Array.isArray(ics)) seen.comments = ics.map((c) => ({ id: Number(c.id), user: login(c.user), body: str(c.body) }));
        if (seen.headSha) {
          const checks = (await get(ctx, paths.checks(seen.headSha), { pull: ref.url })) as { check_runs?: unknown } | undefined;
...
async function pollTaskBranches(ctx: AdapterPollContext): Promise<IngestRequest[]> {
  const open = ctx.snapshot.tasks.filter((t) => t.state === "open" && t.project?.repo);
  ...
  for (const [repo, tasks] of byRepo) {
    ...
    const rows = await get(ctx, `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(name)}/pulls?state=open&per_page=100`, { repo });
...
/** One GitHub read. A failure is logged and answered with undefined. */
async function get(ctx: AdapterPollContext, path: string, meta: Record<string, unknown>): Promise<unknown> {
  const result = await ctx.runAction("connector:connector_proxy", { toolkit: "github", path, method: "GET" })
    .catch((error: unknown) => ({ status: "error" as const, error: error instanceof Error ? error.message : String(error) }));
  if (result.status !== "ok") {
    ctx.log.warn("github read failed", { ...meta, path, reason: result.status === "error" ? result.error : result.status });
    return undefined;
  }
