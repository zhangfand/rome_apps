...
class TemplateApiHandler implements RomeAppApiHandler {
  ...
  async handle(request: RomeAppApiRequest): Promise<Response> {
    ...
    if (request.method === "POST" && route === "run-hello") {
      ...
      const result = await this.ctx.runAction("family-health:hello", { message });
      return json(result);
    }
    ...
    if (request.method === "POST" && route === "ask") {
      ...
      try {
        const result = await this.ctx.runAction("family-health:ask_agent", { prompt });
        return json(result);
      } catch (err) {
        return json(
    ...
    return json(
      {
        error: "not_found",
        appId: this.ctx.app.id,
        message: `Unknown Family Health API route: /${route}`,
      },
      { status: 404 },
    );
  }
