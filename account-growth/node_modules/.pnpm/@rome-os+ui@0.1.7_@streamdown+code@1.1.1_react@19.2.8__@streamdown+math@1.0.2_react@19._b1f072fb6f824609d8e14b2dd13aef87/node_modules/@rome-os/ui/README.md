...
> [`Alert`](#alert), [`Card`](#card), [`Spinner`](#spinner), and
...
Every component gets its own subpath export (`@rome-os/ui/button`) plus a place
in the barrel. Prefer the subpath in app code so a bundler never has to reach
the whole kit to find one component. [`Markdown`](#markdown) is the one
exception with no barrel entry — its Streamdown peers are optional, and a
barrel re-export would make `import { Button } from "@rome-os/ui"` fail to
resolve for anyone who never installed them.
...
Height, padding, radius, and type size come from the host's `--control-*` /
...
- `compact` is a size preset (`text-sm` plus tightened block spacing), not a
  floor: a `className` asking for `text-xs` wins, because the wrapper merges
  through [`cn`](#utilities).
...
The kit ships the **vocabulary**, never the **values**: semantic tokens,
variants, and base-layer defaults come from `@rome-os/ui/styles.css`, while the
host owns the token values (its theme). A host can therefore restyle every kit
component by changing its own custom properties, with no fork of the kit.
...
- **Never a dependency** — heavy tails such as `streamdown` + its
  ...
  pays for a calendar. [`Markdown`](#markdown) is the worked example: optional
  peers, a subpath-only export, and its own opt-in stylesheet.
