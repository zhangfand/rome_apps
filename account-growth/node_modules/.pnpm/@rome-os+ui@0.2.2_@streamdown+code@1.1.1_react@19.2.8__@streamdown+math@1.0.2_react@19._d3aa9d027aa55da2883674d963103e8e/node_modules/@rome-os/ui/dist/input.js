...
const SIZE_MD = "h-[var(--control-h-md)] rounded-[var(--control-r-md)] px-[var(--control-px-start-md)]";
...
function Input({ className, type, size = "md", icon, ...props }) {
    const field = (_jsx("input", { type: type, "data-slot": "input", "data-size": canonicalControlSize(size), className: cn(inputVariants({ size, hasIcon: Boolean(icon) }), className), ...props }));
    if (!icon)
        return field;
    return (_jsxs("div", { className: "relative w-full", children: [_jsx("span", { "aria-hidden": "true", "data-slot": "input-icon", "data-size": canonicalControlSize(size), className: cn(
                ...
                "pointer-events-none absolute inset-y-0 flex items-center text-muted-foreground", iconSizeClass[size ?? "md"], iconInsetClass[size ?? "md"]), children: icon }), field] }));
}
