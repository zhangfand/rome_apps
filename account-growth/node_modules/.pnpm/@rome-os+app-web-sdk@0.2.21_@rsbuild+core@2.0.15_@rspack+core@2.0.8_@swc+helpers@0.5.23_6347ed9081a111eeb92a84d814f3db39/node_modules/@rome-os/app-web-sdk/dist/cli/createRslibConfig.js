...
import { loadConfig, mergeRslibConfig } from "@rslib/core";
import { pluginReact } from "@rsbuild/plugin-react";
import tailwindPostcss from "@tailwindcss/postcss";
import { generateEntry } from "./generateEntry.js";
import { loadAppYaml } from "./loadAppYaml.js";
...
export async function createBuildContext(options) {
    ...
    if (!yaml.appRoot) {
        throw new Error(`rome: ${appDir}/app.yaml must declare "appRoot" (e.g. \`appRoot: dist\`) so build outputs land where the runtime expects them.`);
    }
    const srcDir = join(appDir, "src");
    const outDir = join(appDir, yaml.appRoot);
    if (!existsSync(srcDir)) {
        throw new Error(`rome: source directory ${srcDir} not found. With the new layout, all sources live under <appRoot>/src/.`);
    }
    ...
    if (!existsSync(userEntryPath)) {
        throw new Error(`rome: web entry not found at ${userEntryPath} (set web.entry in app.yaml or create src/web/App.tsx)`);
    }
    ...
    const hasBackendSources = globSync("**/*.{ts,tsx}", {
        cwd: srcDir,
        exclude: [
            `${WEB_DIR}/**`,
            "**/*.d.ts",
    ...
    const baseConfig = {
        ...
        source: {
            tsconfigPath,
            define: {
                "process.env.NODE_ENV": JSON.stringify(options.mode === "development" ? "development" : "production"),
            },
        },
        lib,
    };
    const userConfig = await loadUserConfig(appDir);
    const rslibConfig = userConfig
        ? mergeRslibConfig(baseConfig, userConfig)
        : baseConfig;
    return { cwd, appDir, srcDir, outDir, backendDirs, loaded, rslibConfig };
}
function createBackendLib(args) {
 ...
 */
export function copyBackendAssets(args) {
    const { srcDir, outDir } = args;
    if (!existsSync(srcDir))
        return;
...
function resolveWithin(baseDir, path, label) {
    if (isAbsolute(path)) {
...
//# sourceMappingURL=createRslibConfig.js.map