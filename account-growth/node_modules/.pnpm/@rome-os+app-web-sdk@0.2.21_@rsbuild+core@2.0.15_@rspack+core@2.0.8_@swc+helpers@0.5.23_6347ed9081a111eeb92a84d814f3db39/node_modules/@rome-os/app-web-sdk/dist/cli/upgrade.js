...
export async function runUpgrade(argv) {
    ...
    if (positionals.length > 2) {
        throw new CliError(`rome upgrade: unexpected argument(s): ${positionals.slice(2).join(" ")}`, 2);
    }
    ...
    process.stdout.write(`Updated ${result.manifestPath}: ${result.previousVersion} -> ${result.version}\n`);
}
export async function upgradeAppVersion(startDir, release) {
    const manifestPath = await findManifestPath(startDir);
    const raw = await fs.readFile(manifestPath, "utf8");
    const doc = parseDocument(raw);
    if (doc.errors.length > 0) {
        throw new CliError(`Failed to parse app manifest ${manifestPath}: ${doc.errors[0]?.message ?? "invalid YAML"}`);
    }
...
function parseReleaseType(input) {
    if (RELEASE_TYPES.includes(input)) {
        return input;
    }
    throw new CliError(`rome upgrade: expected major, minor, or patch; got ${JSON.stringify(input)}`, 2);
}
async function findManifestPath(startDir) {
...
function parseSemver(version) {
    ...
    if (!match) {
        throw new CliError(`app.yaml version must be SemVer, for example 1.2.3; got ${JSON.stringify(version)}`);
    }
    ...
    if (!Number.isSafeInteger(major) ||
        ...
        throw new CliError(`app.yaml version is too large to increment safely: ${JSON.stringify(version)}`);
    }
