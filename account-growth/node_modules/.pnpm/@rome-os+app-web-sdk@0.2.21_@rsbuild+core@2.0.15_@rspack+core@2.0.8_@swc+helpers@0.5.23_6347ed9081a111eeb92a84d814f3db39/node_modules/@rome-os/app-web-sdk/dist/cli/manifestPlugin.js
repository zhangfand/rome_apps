...
export class RomeAppManifestPlugin {
    options;
    constructor(options) {
    ...
    apply(compiler) {
        ...
        compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
            compilation.hooks.processAssets.tap({
                ...
                if (!entryChunk) {
                    compilation.errors.push(new webpack.WebpackError(`${PLUGIN_NAME}: missing entry chunk for Rome app web build`));
                    return;
                }
                const entryFile = pickJsFile(entryChunk.files);
                if (!entryFile) {
                    compilation.errors.push(new webpack.WebpackError(`${PLUGIN_NAME}: entry chunk has no JS file`));
                    return;
                }
...
function findEntryChunk(compilation) {
...
function collectStyles(compilation) {
    return Object.keys(compilation.assets)
        .filter((name) => name.endsWith(".css"))
        .sort();
}
function hashAssets(compilation) {
