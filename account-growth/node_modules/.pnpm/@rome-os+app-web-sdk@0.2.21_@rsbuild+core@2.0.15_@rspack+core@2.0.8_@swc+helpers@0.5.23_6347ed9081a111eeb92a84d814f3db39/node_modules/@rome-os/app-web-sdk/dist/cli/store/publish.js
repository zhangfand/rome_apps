...
export async function runPublish(argv) {
    ...
    if (manifestObject.includeSource !== undefined &&
    ...
    if (includeSource) {
        ...
        for (const pattern of SOURCE_EXCLUDES)
            excludes.add(pattern);
    }
    const bytes = await packDirectory(parentDir, dirName, excludes, includeSource);
    const hash = createHash("sha256").update(bytes).digest("hex");
    process.stdout.write(`Packaged ${id}@${version} — ${bytes.length} bytes, sha256 ${hash}, sourceAvailable ${includeSource}\n`);
...
async function validatePublishedSource(sourceDir) {
    const sourceStat = await fs.lstat(sourceDir).catch(() => null);
    if (!sourceStat?.isDirectory() || sourceStat.isSymbolicLink()) {
        throw new CliError(`app.yaml enables includeSource, but ${sourceDir} is not a regular directory`);
    }
    await validatePublishedSourceTree(sourceDir, sourceDir);
}
async function validatePublishedSourceTree(sourceRoot, currentDir) {
    const entries = await fs.readdir(currentDir, { withFileTypes: true });
    for (const entry of entries) {
        if (SOURCE_EXCLUDES.includes(entry.name) || entry.name === ".DS_Store")
            continue;
        const absolutePath = path.join(currentDir, entry.name);
        const relativePath = path.relative(sourceRoot, absolutePath);
        const stat = await fs.lstat(absolutePath);
        if (stat.isSymbolicLink()) {
            throw new CliError(`Cannot publish source file ${relativePath}: symbolic links are not allowed in src/`);
        }
        if (stat.isDirectory()) {
        ...
        if (!stat.isFile() || stat.nlink > 1) {
            throw new CliError(`Cannot publish source file ${relativePath}: only regular, non-linked files are allowed in src/`);
        }
        if (isBlockedSourceFile(entry.name)) {
            throw new CliError(`Cannot publish source file ${relativePath}: secret files are not allowed in src/`);
        }
    }
}
function isBlockedSourceFile(fileName) {
    const normalizedFileName = fileName.toLowerCase();
    if (normalizedFileName === ".env")
        return true;
    if (normalizedFileName.startsWith(".env.") &&
...
async function packDirectory(parentDir, dirName, excludes = new Set(), includeRootSource = true) {
    const chunks = [];
    const stream = tarCreate({
        ...
        filter: (entryPath) => {
            ...
            if (!includeRootSource && segments[0] === dirName && segments[1] === "src")
                return false;
