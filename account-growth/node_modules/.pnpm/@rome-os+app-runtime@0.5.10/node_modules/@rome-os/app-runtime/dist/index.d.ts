...
/** Runtime-owned capabilities available while one Action handler executes. */
export interface ActionExecutionContext {
    emitActionEvent<TEvent extends ActionEvent>(event: TEvent): void;
}
/** One eager Action invocation, exposing public events separately from its
 * ordinary final ActionResult. */
...
export declare class ActionInvocationEventOverflowError extends Error {
...
export declare const REASONING_EFFORT_VALUES: readonly ["low", "high", "xhigh"];
...
export type AgentErrorCode = "usage_limit" | "auth_revoked" | "model_provider_unavailable" | "model_unavailable" | "no_model_provider_available";
export type AgentErrorProvider = "openai" | "anthropic";
export type AgentErrorReason = "not_logged_in" | "quota_exhausted" | "model_access_denied" | "no_available_provider";
...
/** Provider-authored execution plan for one Agent turn (RFC 052).
 *  Rome persists and renders this state but never mutates it. */
...
export type AgentMessage = TurnStartMessage | TurnEndMessage | TextMessage | TextDeltaMessage | ThinkingMessage | ToolUseMessage | ToolResultMessage | SubagentStartMessage | SubagentResultMessage | ResultMessage | ErrorMessage | SessionInitMessage | StructuredOutputMessage | PlanUpdateMessage;
...
export interface ImageGenerationProvider extends ImageGenerationProviderInfo {
    ...
    generate(request: ImageGenerationRequest, ctx?: ImageGenerationContext): Promise<ImageProviderGenerateResult>;
}
...
/** No provider could serve the request; one entry per registered provider with its remedy. */
 | {
    status: "unavailable";
    providers: Array<ImageGenerationProviderInfo & {
        reason: string;
        remedy: string;
    }>;
} | {
...
export interface ImageGenerationInterface {
    ...
    generate(request: ImageGenerationRequest, ctx?: ImageGenerationContext): Promise<ImageGenerationOutcome>;
}
 ...
 * email branch carries the email-only fields. `ChannelPort.sendMessage`
...
/**
 * The message-plane port exposed to apps and hooks. Transport lifecycle is
 * registry-owned, so starting and stopping concrete adapters is intentionally
...
export interface ChannelPort {
    ...
    /**
     * Optional: add a provider-native processing reaction to an inbound message.
     * Returns the provider reaction ID needed for exact removal.
     */
    addProcessingReaction?(messageId: string): Promise<string | null>;
    /** Optional counterpart to addProcessingReaction(). */
    removeProcessingReaction?(messageId: string, reactionId: string): Promise<void>;
    /**
...
export interface ChannelMessageHook {
    register(): void;
    registerPort(adapter: ChannelPort): void;
}
...
export interface RomeAppContext {
    ...
    runAction(name: string, args: Record<string, unknown>, options?: RunActionOptions): Promise<ActionResult>;
    runAction(name: string, args: Record<string, unknown>, options: DetachedRunActionOptions): Promise<ActionDispatchReceipt>;
    invokeAction<TEvent extends ActionEvent = ActionEvent, TOutput = unknown>(name: string, args: Record<string, unknown>): ActionInvocation<TEvent, TOutput>;
 ...
 */
export declare function visitorAuthRequired(message?: string): Response;
 ...
 */
export declare function requireVisitor(request: RomeAppApiRequest, opts?: {
    message?: string;
