...
import { type Fact, type StartedFact } from "../../lib/facts.js";
import {
  HEARTBEAT_LEASE_MS, HEARTBEAT_PROTOCOL, heartbeatState,
...
export class WorkerHealthRepository {
  ...
  claim(started: StartedFact, ownerId: string, now = new Date()): boolean {
    return this.db.transaction((tx) => {
      ...
      if (!current || current.payload.heartbeatProtocol !== HEARTBEAT_PROTOCOL) return false;
  ...
  expire(taskId: string, workerId: string, now = new Date()): Fact | undefined {
    return this.db.transaction((tx) => {
      ...
      if (!started || started.payload.heartbeatProtocol !== HEARTBEAT_PROTOCOL) return undefined;
      ...
      return ledger.appendWorkerOutcome({
        taskId, kind: "Lost", by: "runtime", source: "manager:reconcile, observing worker heartbeat",
...
function activeStart(ledger: LedgerRepository, taskId: string, workerId: string): StartedFact | undefined {
  const facts = ledger.factsFor(taskId);
  if (!facts.length) return undefined;
  const task = foldTask(facts);
  if (task.state !== "taken" || task.liveWorker?.workerId !== workerId) return undefined;
  return facts.find((fact): fact is StartedFact => fact.kind === "Started" && fact.payload.workerId === workerId);
}
